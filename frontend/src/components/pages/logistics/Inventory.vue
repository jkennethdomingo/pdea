...
// Dummy product data (consider fetching from an API)
const products = ref(Array.from({ length: 10 }, () => ({
  name: 'Apple iMac 27"',
  category: 'PC',
  brand: 'Apple',
...
let ps = null;
  ...
  onMounted(() => {
    if (scrollContainerRef.value) {
      ps = new PerfectScrollbar(scrollContainerRef.value);
    }
    initDropdowns();
  });
...
<template>
    ...
    <div ref="scrollContainer" class="scroll-container mx-auto max-w-screen-xl px-4">
        <!-- Start coding here -->
        <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
            <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                ...
                <div class="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
                    <button type="button" class="flex items-center justify-center text-white bg-green-500 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800">
...
.scroll-container {
  position: relative;
  max-height: 72vh; /* Or whatever max-height you desire */
  overflow-y: scroll;
}
...
@media (min-width: 1024px) { 
    .scroll-container {
    position: relative;
    max-height: 76vh; /* Or whatever max-height you desire */
    overflow-y: scroll;
}
 }
/* Your other styles */
